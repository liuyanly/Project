...
export default {
    ...
    created(){
        ...
        // this.token = '277a25171277beb000290d9f1eecd995'; //测试数据
        ...
        isCheck(e,index) {
            ...
            if(this.tmpData.length>0){
                    for(let key in this.tmpData){
                         // 一个知识点相同，id不同的时候提示每个知识点只能选择一道题
                        // if(this.tmpData[key].id!=e.id){
                         // if(this.tmpData[key].id!=e.id&&this.tmpData.length>0&&this.flagTmp&&this.oneFlag){
                            // console.log('aaaaaa')
                            // this.flagTmp=false;
                            // this.$toast("每个知识点只能选择一道题");
                        // }
            ...
            //     if(this.tmpData[i].third_knowledge_id){
            //         console.log(11111)
            //         for (var i = 0; i < this.tmpData.length; i++) {
            //             if(this.tmpData[i].id!=e.id){
