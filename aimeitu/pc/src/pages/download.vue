    ...
    <div class="warpper">
        ...
        <div class="header">
            <div class="container clearfix">
                ...
                <div class="nav">
                    <router-link :to="{name: 'home'}">首页</router-link><router-link :to="{name: 'picture'}">图片</router-link><router-link :to="{name: 'video'}">视频</router-link><router-link :to="{name: 'about'}">关于我们</router-link><router-link :to="{name: 'download'}" class="active">下载APP</router-link>
                </div>
            </div>
        </div>
        <div class="appbox">
...
export default {
    ...
    methods: {
        getData(){
            let url =this.$util.baseset.baseurl+'/platform/getClauseByTag?tag=pc_download';
            this.$ajax.get(url)
            .then((res) => {
                let data = res.data;
                if(data.ret === 1){
