<template>
    <div class="warpper">
        ...
        <div class="subnav">
            <div class="container">
                <span v-for="category in categorys"><a href="#" @click="categorysTab(category.id, category.name)" :class="{'active': isActive == category.id}">{{category.name}}</a><i>|</i></span>
            </div>
        </div>
        <div class="container">
...
export default {
    ...
    created(){
     ...
     methods : {
        ...
        getTempData(id) {
            let url = this.$util.baseset.baseurl+'/dynamic/getCategoryDynamic?type='+1+'&categoryId='+id+'&start='+this.currentPage+'&lenght='+this.pageSize;
            this.$ajax.get(url)
            .then((res) => {
                var data = res.data;
                this.pictures = [];
