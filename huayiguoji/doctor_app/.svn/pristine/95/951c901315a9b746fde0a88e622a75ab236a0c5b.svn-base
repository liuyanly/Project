		...
		<div class="contentScollr">
			<div class="form-box">
				<div class="form">
					<ul>
						<li>
							<textarea maxlength="200" placeholder="请输入详细内容" v-model="goodAt" @keyup="numChange()">
							</textarea>
...
export default {
  	...
  	methods:{
		...
		numChange(){
			this.wordNum = 200 - this.goodAt.length
		},
		complete(){
			if(this.goodAt==''){
				this.$toast('请输入擅长的内容');
			...
			window.sessionStorage.setItem('specialty',this.goodAt)
			this.$router.push({name:'MyData'})
		},
  	},
	mounted:function(){
		if(sessionStorage.getItem('specialty')){
			this.goodAt = sessionStorage.getItem('specialty')
			this.numChange()
		}
  	}
}
