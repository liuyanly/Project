        ...
        <div class="contentScollr">
            <div class="info">
                ...
                <div class="content">
                    <ul>
                        <li>
                            <span class="name">姓名</span>：{{userInfo.realname}}
                            <span class="tag">（{{translates.sex}}，{{userInfo.translates.age}}岁）</span>
                        </li>
                        <li>
            ...
            <div class="describe" v-for="data in descri">
                ...
                <div class="content">
                    <div class="txt">
                        ...
                        <span v-html="data.pivot.value"></span>
                    </div>
                </div>
            </div>
            <div class="data_box">
                ...
                <ul v-if="abnormalData.length>0">
                    <li v-for="(data,index) in abnormalData" @click="testData(aId,data.name)">{{data.name}}
                        <span class="tag" v-if="data.count>0">{{data.count}}</span>
                    </li>
                </ul>
            ...
            <div class="photo">
                ...
                <ul v-if="photourl.length != 0">
                    ...
                    <li v-for="(item, key) in photourl" :key="key">
                        <img v-preview="item" :src="item" :key="key" preview-title-enable="true" preview-nav-enable="true">
...
export default {
  ...
  methods: {
    testData(id, title) {
      this.$router.push({
        name: "TestData",
        params: { id: this.id, fieldId: id, title: title }
      });
    },
    getData() {
      var that = this;
          ...
          if (res.data.status == 200) {
            ...
            if (res.data.data.detection) {
              for (var i = 0; i < res.data.data.detection.length; i++) {
                ...
                for (
                  var j = 0;
                  j < res.data.data.detection[i].data.length;
                  j++
                ) {
                  tem.count +=
                    res.data.data.detection[i].data[j].abnormal_count;
                }
                that.abnormalData.push(tem);
              }
            }
            that.photourl = res.data.data.translates.images;
            that.telNum = res.data.data.user.user_account.username;
            if (
              res.data.data.archivedatas instanceof Array &&
              res.data.data.archivedatas.length > 0
            ) {
              if (res.data.data.archivedatas[0].translates.fields) {
                for (
                  var i = 0;
                  i < res.data.data.archivedatas[0].translates.fields.length;
                  i++
                ) {
                  if (
                    res.data.data.archivedatas[0].translates.fields[i].type == 0
                  ) {
                  } else {
                    that.descri.push(
                      res.data.data.archivedatas[0].translates.fields[i]
                    );
                  }
                }
              }
