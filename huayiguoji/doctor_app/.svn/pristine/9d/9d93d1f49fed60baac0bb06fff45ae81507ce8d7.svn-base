<template>
	<div class="intervention">
        ...
        <div class="head_box" @click="openPicker()">
			...
			<span @click="publisthInter()" class="publish-btn">发布干预</span>
        </div>
...
export default {
  ...
  methods:{
	  publisthInter(){
		  this.$router.push({name:'PublishIntervention',params:{id:0,title:'发布干预'}})
	  },
	  ...
	  getDataList(){
		  ...
		  that.$ajax.get('/intervene/user/'+that.id,{params:params})
		  .then(function(res){
			  if(res.data.status == 200){
				  if(res.data.data.paginate.total_page==0){
					  that.$toast('暂无数据')
				  }else{
					  for(var i=0; i<res.data.data.intervene.length;i++){
						  that.dataList.push(res.data.data.intervene[i])
					  }
