<template>
  <div class="consultChild">
    ...
    <div class="operat" v-if='type==0&&status==3'>
      <div class="flex">
        <button class="suggestion" @click="sureComplete()">拨打电话</button>
      </div>
    ...
    <div class="operat" v-if='type==1&&status==3'>
      <div class="flex">
        <button class="suggestion" @click="beginVideo()">发起视频</button>
      </div>
    </div>
  </div>
</template>
...
<script>
export default {
  props: ["type", "info", "status", "id", "user"],
  mounted() {},
  methods: {
    sureComplete() {
      var that = this;
      this.$ajax
        .post("user/anonymousphone", { service_id: 2 })
        .then(res => {
      ...
      this.$ajax
        .put("servicefinish/" + this.id, { status: 1 })
        .then(res => {
    ...
    beginVideo() {
      ...
      this.native.live({
        ...
        jobtitle: that.user.jobtitle,
        department: that.user.department,
        hospital: that.user.hospital,
        identity: "user"
      });
    }
  }
