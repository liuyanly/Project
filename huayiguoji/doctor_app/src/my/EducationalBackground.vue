		...
		<div class="contentScollr">
			<div class="form-box">
				<div class="form">
					<ul>
						<li>
							<textarea maxlength="200" placeholder="请输入详细内容" v-model="educationalBackground">
							</textarea>
  	...
  	methods:{
		...
		complete(){
			if(this.educationalBackground==''){
				this.$toast('请输入您的教育背景');
				return false;
			}
			window.sessionStorage.setItem('background',this.educationalBackground)
			this.$router.push({name:'MyData'})
		},
  	},
	mounted:function(){
		if(sessionStorage.getItem('background')){
			this.educationalBackground = sessionStorage.getItem('background')
		}
  	}
}
</script>
