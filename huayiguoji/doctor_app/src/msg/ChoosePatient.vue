...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            serviceList(id){
                this.$router.push({name:'PublishMsg',params:{patientId:id}});
            },
            getList(){
                var that = this
                that.$ajax.get('/user/patient?per_page='+that.per_page+'&page='+that.currentPage)
                .then(function(res){
                    if(res.data.status==200){
                        if(res.data.data.paginate.total_page==0){
        					that.$toast('暂无数据')
        				}else{
        					for(var i=0; i<res.data.data.patient.length;i++){
        						that.dataList.push(res.data.data.patient[i])
        					}
