<template>
  <div class="im-dialog">
    ...
    <div class="iminput">
      <div class="addImg">
        <button @click="addImg"><img src="../assets/images/e2_tianjia.png" alt=""><input type="file" name="singleFile" id="singleFile" style="display:none" @change="uploadPic()"></button>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.getUserInfo();
    var userInfo = JSON.parse(localStorage.doctorInfo);
    loginInfo = {
      sdkAppID: sdkAppID, //用户所属应用id,必填
      identifier: userInfo.id, //当前用户ID,必须是否字符串类型，必填
      accountType: accountType, //用户所属应用帐号类型，必填
      userSig: userInfo.im_sig, //当前用户身份凭证，必须是字符串类型，必填
      identifierNick: userInfo.realname, //当前用户昵称，不用填写，登录接口会返回用户的昵称，如果没有设置，则返回用户的id
      headurl: userInfo.avator //当前用户默认头像，选填，如果设置过头像，则可以通过拉取个人资料接口来得到头像信息
    };
    this.doctorName = userInfo.realname;
    this.doctorJob = userInfo.jobtitle;
    this.doctorDept = userInfo.translates.department_title;
    this.hospitalName = userInfo.translates.hospital_title;
    this.doctorAvator = userInfo.translates.avatar_img;
    webim.login(
      loginInfo,
      listeners,
      options,
