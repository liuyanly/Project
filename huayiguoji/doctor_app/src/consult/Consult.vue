<template>
    <div class="consult">
        ...
        <Child :type='consultType' :info='info' :user='user' :id='id' :status='status' />
    </div>
</template>
<script>
      ...
      this.$ajax
        ...
        .then(res => {
          if (res.data.status == 200) {
            ...
            this.user.nickname = res.data.data.user.realname;
            this.user.avatar = res.data.data.user.translates.avatar_img;
            this.user.jobtitle = res.data.data.doctor.jobtitle;
            this.user.department =
              res.data.data.doctor.translates.department_title;
            this.user.hospital = res.data.data.doctor.translates.hospital_title;
            this.info.startTime = res.data.data.started_at.substring(0, 16);
            this.info.duration =
              res.data.data.service_details.translates.duration;
          }
        })
        .catch(err => {
          this.BaseSet.linkLogin(this);
