        ...
        <ul class="order-list already-complete">
            <li class="clear" @click="serviceList(data.id)" v-for="data in dataList">
                ...
                <div class="left info-box">
                    ...
                    <p class="user-info">
                        {{data.translates.sex}}&emsp;{{data.translates.age}}岁
                    </p>
                    <p class="status">
                        症状：{{data.illness}}
                    </p>
...
<script>
    ...
    export default {
        ...
        mounted(){
            this.getDataList('/user?per_page='+this.per_page+'&page='+this.currentPage)
        },
        methods:{
            loadBottom() {
            ...
            serviceList(id){
                if(this.type==1){
                    this.$router.push({name:'SetUpVip',params:{id:id}});
                }else if(this.type==2){
                    this.$router.push({name:'PatientDetail',params:{type:1,patientId:id,userId:this.$route.params.userId}});
                }else{
                    this.$router.push({name:'PatientDetail',params:{type:2,patientId:id,userId:this.$route.params.userId}});
                }
            },
            getDataList(url){
                var that=this
