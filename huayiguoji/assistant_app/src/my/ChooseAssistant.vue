        ...
        <ul class="order-list already-complete">
            <li class="clear" @click="serviceList(data.id)" v-for="data in dataList">
                ...
                <div class="left info-box">
                    ...
                    <p class="status" v-if="type!=2">
                        服务患者：{{data.service_num}}人
                    </p>
                    <p class="status" v-if="type==2">
                        已签约：{{data.service_num}}人
                    </p>
                </div>
            </li>
        </ul>
...
<script>
    ...
    export default {
        ...
        mounted(){
            this.getDataList('/assistant?per_page='+this.per_page+'&page='+this.currentPage)
        },
        methods:{
            loadBottom() {
            ...
            serviceList(id){
                if(this.$route.params.type==1){
                    this.$router.replace({name:'DistributionTask',params:{patientId:this.$route.params.patientId,userId:id}});
                    // this.$router.push({name:'ChooseAssistantDetail',params:{type:1,patientId:this.$route.params.patientId,assistantId:id}});
                }else{
                    this.$router.replace({name:'SignDistribution',params:{patientId:this.$route.params.patientId,userId:id}});
                    // this.$router.push({name:'SignAssistantDetail',params:{type:2,patientId:this.$route.params.patientId,assistantId:id}});
                }
            },
            getDataList(url){
                var that=this
