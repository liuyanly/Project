<template>
	<div class="publish-box">
		...
		<div class="user-info clear doctor-info" v-if="doctorId!=0" @click="ChooseDoctor()">
			<img :src="doctorImg" class="left" />
			<div class="left">
				<p class="name">
					<span>{{doctorData.realname}}</span>
					<span class="disease">&emsp;{{doctorDepartment}}</span>
				</p>
			</div>
		</div>
		<div class="user-info clear" v-if="doctorId==0" @click="ChooseDoctor()">
			<p class="tip">
				请选择医生
			</p>
		</div>
...
time = d.getFullYear()+'年'+(d.getMonth()+1)+'月'+d.getDate()+'日'+d.getHours()+'点'+d.getMinutes()+'分'
	...
	export default {
		...
		mounted(){
			...
			this.contentWord = this.BaseSet.getInfo('temContent')
			if(this.patientId != 0){
				this.getPatient()
			}
			if(this.doctorId != 0){
		...
		methods:{
			...
			ChoosePatient(){
				this.$router.push({name:'ChoosePatient',params:{doctorId:this.doctorId,temId:this.temId,patientId:this.patientId}})
			},
			chooseTem(){
				this.$router.push({name:'ChooseTemplate',params:{patientId:this.patientId,doctorId:this.doctorId,temId:this.temId}})
			},
			ChooseDoctor(){
				this.$router.push({name:'ChooseDoctor',params:{patientId:this.patientId,temId:this.temId,doctorId:this.doctorId}})
			},
			getPatient(){
                var that = this
                that.$ajax.get('/user/'+that.patientId)
                .then(function(res){
			...
			getDoctor(){
                var that = this
                that.$ajax.get('/user/'+that.doctorId)
                .then(function(res){
                    if(res.data.status == 200){
                        that.doctorData = res.data.data
						that.doctorImg = res.data.data.translates.avatar_img
						that.doctorDepartment = res.data.data.translates.department_title
                    }
                })
                .catch(function(err){
                    that.BaseSet.linkLogin(that)
			...
			publish(){
				...
				var dt = {
					...
					useraccount:that.patientId,
					doctoraccount:that.doctorId,
					tosms:that.radioVal,
					send_at:that.pickerValue
				}
				that.$ajax.post('/user/sendnotifications',dt)
				.then(function(res){
					if(res.data.status == 200){
						that.$toast('消息发布成功')
