<template>
	<div class="follow_box">
		...
		<router-link :to="{ name: 'PublishFollow', params: {id:0,title:'请选择随访类型'} }">
			<p class="publisth-btn">
				发布随访
			</p>
		</router-link>
		<div class="contentScollr">
			...
			<mt-datetime-picker ref="picker" type="date" :endDate="endDate" year-format="{value}" month-format="{value}" date-format="{value}" @confirm="handleConfirm">
			</mt-datetime-picker>
		</div>
	</div>
</template>
...
export default {
  data() {
    return {
      ...
      allLoaded: false, //分页
      scrollMode: "auto",
      id: this.BaseSet.getInfo("patientId")
    };
  },
  mounted() {
