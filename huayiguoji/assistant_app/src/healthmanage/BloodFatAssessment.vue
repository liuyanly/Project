			...
			<div class="risk">
				...
				<div class="risk-box">
					...
					<div class="thermometer-chart relative">
						...
						<div class="current absolute clearFix" v-bind:style="{bottom: dataBlnum+'%'}"><span class="me left"><span>我</span></span><span class="num left">{{dataBL.risk_stratification}}</span><span class="status left">({{blStr}})</span></div>
					</div>
				</div>
			</div>
			<div class="chart-box">
...
let d = new Date(),
today = (d.getFullYear())+'-'+((d.getMonth()+2)>9?d.getMonth()+1:'0'+(d.getMonth()+1))+'-'+(d.getDate()>9?d.getDate():'0'+d.getDate());
...
export default {
  ...
  methods:{
    ...
    getData(){
		var that = this
		that.$ajax.get('/check/bloodfat/'+that.patientId)
		.then(function(res){
