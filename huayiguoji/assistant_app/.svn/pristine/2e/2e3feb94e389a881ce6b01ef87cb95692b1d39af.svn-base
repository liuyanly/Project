<template>
	<div class="monitoring_detail">
		...
		<div class="contentScollr">
			<div class="nav">
				<ul id="nav">
					<li v-for="(nav,index) in navItem" :key="index" @click="SubCates(nav.name,nav.id,nav.title)" :class="{'active': isActive == nav.id}">
						{{nav.title}}
					</li>
				</ul>
			</div>
			<div class="time_search">
				<div class="time">
					<span class="txt" @click="openPicker(1)">{{pickerValue}}</span><img src="../assets/images/c12_xiala.png" /></div>
				~
				<div class="time">
					<span class="txt" @click="openPicker(2)">{{pickerValue1}}</span><img src="../assets/images/c12_xiala.png" /></div>
				<div class="btn" @click="filterData()">查询</div>
			</div>
			<div class="chart-box">
				<p class="chart-name">
					{{monitoringtitle}}变化趋势
				</p>
			...
			<mt-datetime-picker ref="picker1" type="date" :endDate="endDate" year-format="{value}" month-format="{value}" date-format="{value}" @confirm="handleConfirm1">
			</mt-datetime-picker>
		</div>
	</div>
</template>
...
export default {
  ...
  methods: {
          ...
          if (res.data.status == 200) {
            if (res.data.data instanceof Array) {
              ...
              for (var i = 0; i < that.navItem.length; i++) {
                if (that.navItem[i].id == that.isActive) {
                  that.monitoringtitle = res.data.data[i].title;
                  break;
                }
              }
            }
    ...
    SubCates(val, index, title) {
      this.monitoringtitle = title;
      this.content = val;
      this.isActive = index;
      this.filterData();
    },
