...
export default {
  ...
  mounted() {
    this.url =
      "/user/assistant_doctor?per_page=" +
      this.per_page +
      "&page=" +
      this.currentPage;
    this.getDepartment();
  ...
  methods: {
    ...
    searchBtn() {
      ...
      if (this.searchChangeVal != "") {
        this.searchVal = this.searchChangeVal;
        this.getList();
      }
    },
    viewDetail(id) {
      this.$router.push({
        name: "PublishMsg",
        params: { patientId: this.patientId, doctorId: id, temId: this.temId }
      });
    },
    ...
    getList() {
      ...
      that.dataList = [];
      var url =
      "/user/assistant_doctor??per_page=" + this.per_page + "&page=" + this.currentPage;
      if(that.cityId){
